...
for digital image correlation"""
     ...
     def interpolate_displacement(self, point, disp, *args, **kwargs):
          """Interpolate the displacement field to construct a smooth displacement grid."""
          ...
          valid_indices = ~np.isnan(x) & ~np.isnan(y) & ~np.isnan(dx) & ~np.isnan(dy)
          x = x[valid_indices]
          y = y[valid_indices]
          dx = dx[valid_indices]
          dy = dy[valid_indices]
          ...
          if method == 'delaunay':
               ...
               inter_x = LinearNDInterpolator(np.vstack((x, y)).T, dx)
               inter_y = LinearNDInterpolator(np.vstack((x, y)).T, dy)
               self.disp_x = inter_x(self.grid_x, self.grid_y)
               self.disp_y = inter_y(self.grid_x, self.grid_y)
          elif method == 'raw':
               # Direct assignment without interpolation
               self.disp_x = np.zeros_like(self.grid_x)
               self.disp_y = np.zeros_like(self.grid_y)
               self.disp_xy_indi = np.zeros_like(self.grid_x)

               assert self.disp_x.size == len(dx), "Mismatch in displacement data size."
               count = 0
               for i in range(self.disp_x.shape[0]):
                    for j in range(self.disp_x.shape[1]):
                         self.disp_x[i, j] = dx[count]
                         self.disp_y[i, j] = dy[count]
                         self.disp_xy_indi[i, j] = np.hypot(dx[count], dy[count])
                         count += 1
          elif method == 'spline':
               # Spline interpolation
               tck_x = scipy.interpolate.bisplrep(x, y, dx, kx=3, ky=3)
               self.disp_x = scipy.interpolate.bisplev(self.grid_x[:, 0], self.grid_y[0, :], tck_x)
               tck_y = scipy.interpolate.bisplrep(x, y, dy, kx=3, ky=3)
               self.disp_y = scipy.interpolate.bisplev(self.grid_x[:, 0], self.grid_y[0, :], tck_y)
          else:
               # Other interpolation methods ('linear', 'cubic', etc.)
               ...
               # Fill NaN values after interpolation
               self.disp_x = np.nan_to_num(self.disp_x)
               self.disp_y = np.nan_to_num(self.disp_y)

          # Compute the magnitude of displacement
          self.disp_xy_indi = np.hypot(self.disp_x, self.disp_y)
